...
class ClothCompEnv(FlexEnv):
    def __init__(self, observation_mode, action_mode, num_picker=2, render_mode='particle', picker_radius=0.05, picker_threshold=0.005,
        ...
        if action_mode == 'picker':
            self.action_tool = Picker(num_picker, picker_radius=picker_radius, particle_radius=particle_radius, picker_threshold=picker_threshold,
                                      picker_low=(-1, 0., -1), picker_high=(1.0, 1.0, 1))
    ...
    def _sample_cloth_size(self):
        ...
        # Changed according to the info provided by the competition
        # https://github.com/Victorlouisdg/cloth-competition/discussions/19
        min_particles_width = int(0.45 / self.cloth_particle_radius)
        max_particles_width = int(0.75 / self.cloth_particle_radius)

        min_particles_height = int(0.35 / self.cloth_particle_radius)
        max_particles_height = int(0.65 / self.cloth_particle_radius)

        return np.random.randint(min_particles_width, max_particles_width), np.random.randint(min_particles_height, max_particles_height)

    def _sample_cloth_prop(self):
        return [np.random.uniform(0.5, 1.1), np.random.uniform(0.7, 1.3), np.random.uniform(0.6, 1.2)]
    ...
    def get_default_config(self):
        ...
        if self.action_mode in ['sawyer', 'franka']:
            ...
            # Modified by Sun Haoran, set the camera pose same as in the competition:
        ...
        config = {
            ...
            'ClothSize': [int(0.6 / particle_radius), int(0.368 / particle_radius)], # [int(0.6 / particle_radius), int(0.368 / particle_radius)],
